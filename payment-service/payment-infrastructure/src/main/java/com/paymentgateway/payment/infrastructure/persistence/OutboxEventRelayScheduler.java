...
public class OutboxEventRelayScheduler {
    ...
    public void relay() {
        List<UUID> ids = repository.findClaimable(PageRequest.of(0, batchSize))
                .stream().map(OutboxEventJpaEntity::getEventId).toList();
        if (ids.isEmpty()) return;

        transactions.claim(ids, Instant.now(), instanceId);

        List<OutboxEventJpaEntity> claimed = repository.findByClaimedByAndPublishedAtIsNull(instanceId);
        for (OutboxEventJpaEntity event : claimed) {
            publisher.publish(event);
            transactions.acknowledge(event.getEventId(), Instant.now());
        }
    }
